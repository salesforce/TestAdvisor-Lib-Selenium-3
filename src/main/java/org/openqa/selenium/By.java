 ...
 * the protected methods as required, though it is expected that all subclasses
 * rely on the basic finding mechanisms provided through static methods of this
 * class:
 *
 * <code>
...
public abstract class By {
	 ...
	 */
	public <T extends WebElement> T findElement(SearchContext context) {
		List<T> allElements = findElements(context);
	...
	public static class ByClassName extends By implements Serializable {
		...
		/**
		 * Generate a partial XPath expression that matches an element whose specified
		 * attribute contains the given CSS word. So to match &lt;div class='foo
		 * bar'&gt; you would say "//div[" + containingWord("class", "foo") + "]".
		 *
	...
	public static class ByCssSelector extends By implements Serializable {
		...
		public <T extends WebElement> T findElement(SearchContext context) {
			...
			throw new WebDriverException("Driver does not support finding an element by selector: " + cssSelector);
		}
		...
		public <T extends WebElement> List<T> findElements(SearchContext context) {
			...
			throw new WebDriverException("Driver does not support finding elements by selector: " + cssSelector);
		}
