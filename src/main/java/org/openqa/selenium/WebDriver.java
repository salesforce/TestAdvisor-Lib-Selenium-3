 ...
 * browser. The methods in this interface fall into three categories:
 * <ul>
 ...
 * {@link WebDriver#findElement(By)}, which is used to find {@link WebElement}s.
 * <p>
 * Currently, you will need to instantiate implementations of this interface
...
public interface WebDriver extends SearchContext {
	 ...
	 *
	 * @return A {@link org.openqa.selenium.WebDriver.Navigation} that allows the
	...
	interface Options {
		 ...
		 * assumed that the cookie is meant for the domain of the current document.
		 *
		...
		/**
		 * @return the interface for controlling IME engines to generate complex-script
		 *         input.
		 */
		ImeHandler ime();
	...
	interface Timeouts {
		 ...
		 * {@link NoSuchElementException}. When searching for multiple elements, the
		 * driver should poll the page until at least one element has been found or this
		 * timeout has expired.
		 * <p>
	...
	interface TargetLocator {
		 ...
		 */
		<T extends WebElement> WebDriver frame(T frameElement);
	...
	interface ImeHandler {
		 ...
		 * getActiveEngine may still return the name of the engine but isActivated will
		 * return false.
		 *
		 * @throws ImeNotAvailableException if the host does not support IME.
		 */
		 ...
		 * getAvailableEngines) active. After this call, the only loaded engine on the
		 * IME daemon will be this one and the input sent using sendKeys will be
		 * converted by the engine. Note that this is a platform-independent method of
		 * activating IME (the platform-specific way being using keyboard shortcuts).
		 *
		 *
	...
	interface Window {
		 ...
		 * dimension, not just the view port, synonymous to window.resizeTo() in JS.
		 *
		 ...
		 * corner of the screen, synonymous to window.moveTo() in JS.
		 *
